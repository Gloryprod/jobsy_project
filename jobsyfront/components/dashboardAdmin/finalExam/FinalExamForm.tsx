...
// Interface adaptée pour useFieldArray (tableau d'objets au lieu de string[])
interface QuestionFormData {
  id?: number;
  question_text: string;
...
// Interface pour les données brutes venant de l'API / parent (si elles sont en string[])
...
interface QuestionFormProps {
  courseId: string;
  initialData?: RawQuestionData; // Présent si on est en mode édition
  onSuccess?: () => void;         // Callback pour rafraîchir la liste SWR parente
  onCancel?: () => void;
}
...
export default function FinalExamQuestionForm({ courseId, initialData, onSuccess, onCancel }: QuestionFormProps) {
  ...
  // Helper pour transformer les options string[] de l'API en { value: string }[] pour React Hook Form
  const formatInitialValues = (data?: RawQuestionData): QuestionFormData | undefined => {
  ...
  const { register, handleSubmit, control, reset, watch, setValue } = useForm<QuestionFormData>({
    defaultValues: {
      question_text: "",
      options: [{ value: "" }, { value: "" }], // Au moins 2 options par défaut
      correct_answers: [],
      points: 1,
    },
    values: formatInitialValues(initialData), // Injection et formatage automatique
  });
  ...
  // Reset du formulaire si initialData change
  useEffect(() => {
    if (initialData) {
      reset(formatInitialValues(initialData));
    }
  }, [initialData, reset]);
  ...
  // Soumission du formulaire
  const onSubmit = async (formData: QuestionFormData) => {
    // Extraction et nettoyage des objets { value: string } vers des simples string[]
  ...
  // Classes de styles réutilisables
  const inputClass = "w-full mt-1 p-3 bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-2 focus:ring-[#000080]/20 focus:border-[#000080] outline-none transition-all font-medium";
  const labelClass = "block text-xs font-black text-slate-500 uppercase tracking-wider";
    ...
    <div className="p-6 md:p-8 w-full mx-auto animate-fadeIn">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
        ...
        {onCancel && (
          <button 
            type="button"
            onClick={onCancel} 
            className="cursor-pointer text-xs font-bold text-slate-400 hover:text-slate-900 flex items-center gap-1 transition-all"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Annuler
          </button>
        )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2.5">
            {fields.map((field, index) => {
              const currentOptionValue = watchedOptions[index]?.value || "";
              const isCorrect = watchedCorrectAnswers.includes(currentOptionValue) && currentOptionValue.trim() !== "";
              ...
              return (
                ...
                >
                  ...
                  {/* Champ texte de l'option (lié avec .value) */}
                  <input
