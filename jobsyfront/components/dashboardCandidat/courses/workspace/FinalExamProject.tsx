...
import { Clock, Send, CheckCircle2, AlertTriangle, FileText, Link as LinkIcon, Upload, Eye, Save, PlayCircle, Lock, RefreshCw } from "lucide-react";
...
const EXTRA_TIME_MINUTES = 10; // 30 minutes de grâce accordées automatiquement
...
export default function FinalExamProjectPlay({
  ...
  const updateAnswers = (
    newAnswers: Record<string, string> | ((prev: Record<string, string>) => Record<string, string>)
  ) => {
    setAnswers((prev) => {
  ...
  // --- TRAITEMENT AUTOMATIQUE À LA FIN DU DELAI EXTRA ---
  const handleFinalDeadlineReached = useCallback(async () => {
    ...
    const currentProject = projectRef.current;
     ...
     // Si la session ou le projet ne sont pas encore chargés depuis le backend, on ne fait rien
    if (!currentSession || !currentProject || !currentAnswers) {
      console.warn("Attente du chargement complet de la session avant de traiter l'expiration.");
      return;
    }
  ...
  const calculateDoublePhaseTime = useCallback(
    (startedAtStr: string, limitMinutes: number) => {
      if (timerRef.current) clearInterval(timerRef.current);
      ...
      const updateTimer = () => {
        ...
        //   elapsedTimeMinutes: elapsedTimeMs / 1000 / 60,
        //   normalLimitMs,
        //   extraLimitMs,
        //   isExtraPhase: elapsedTimeMs >= normalLimitMs && elapsedTimeMs < extraLimitMs
        // });
        ...
        if (elapsedTimeMs < normalLimitMs) {
          ...
          setTimeLeft(0);
          setIsExtraTime(true);
          // N'exécuter que si les données de session/projet sont bien là
          if (sessionRef.current && projectRef.current) {
  ...
  // --- INITIALISATION DES DONNÉES DU BROUILLON ---
  ...
  const handleStartExam = async () => {
    ...
    if (is_permanently_blocked) {
      return toast.error("Vous avez atteint la limite maximale d'essais pour cet examen.");
    }
    ...
    try {
      ...
      if (project.time_limit_minutes) {
        calculateDoublePhaseTime(newSession.started_at, project.time_limit_minutes);
      }
    } catch (error: any) {
  ...
  // --- LOGIQUE RETRY (24H) ---
  // useEffect(() => {
  //   if (!time_remaining || time_remaining === timeLeftRetry) return;

  //   const timeout = setTimeout(() => {
  //     setTimeLeftRetry(time_remaining);
  //   }, 0);

  //   return () => clearTimeout(timeout);
  // }, [time_remaining, timeLeftRetry]);
  ...
  useEffect(() => {
    if (time_remaining) {
      console.log("Mise à jour du temps restant pour le retry :", time_remaining); 
  ...
  const isBlockedByTimer = can_retry === false && timeLeftRetry > 0 && !is_permanently_blocked;

  const handleRetryExam = async () => {
    if (is_permanently_blocked) {
      return toast.error("Vous ne pouvez plus passer cet examen.");
    }
    ...
    try {
      ...
      // Mettre à jour le state local directement si l'API retourne la nouvelle session
      if (response.data?.session) {
        setSession(response.data.session);

        // Si la nouvelle session est en cours, réinitialiser le chrono
        if (response.data.session.status === "in_progress" && project?.time_limit_minutes) {
          calculateDoublePhaseTime(
            response.data.session.started_at,
            project.time_limit_minutes
          );
        }
      } else {
        // Sinon, recharger les données de l'examen sans recharger toute la page web
        await fetchExamAndSession();
      }
      ...
      <div className="p-8 text-center bg-red-50 border border-red-100 rounded-2xl max-w-lg mx-auto my-8">
        ...
        {onBackToCourse && (
          <button
            ...
            className="mt-4 px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-slate-800 transition-all cursor-pointer"
          >
            Retourner au cours
          </button>
        )}
  ...
  // --- RENDU 0 : ACCUEIL / AVANT COMMENCER ---
  if (!session) {
    return (
      <div className="p-6 md:p-12 w-full max-w-3xl mx-auto my-8 bg-white border border-slate-100 rounded-3xl shadow-xs text-center space-y-6">
        <div className="w-16 h-16 bg-[#000080]/10 text-[#000080] rounded-full flex items-center justify-center mx-auto">
          {is_permanently_blocked ? <Lock className="w-8 h-8 text-red-500" /> : <PlayCircle className="w-8 h-8" />}
        </div>
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            {attempts_count > 0 && (
              <span className="px-3 py-1 bg-slate-100 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-full">
                {attempts_count} {attempts_count > 1 ? "tentatives effectuées" : "tentative effectuée"}
              </span>
            )}
          </div>
        ...
        {/* ALERTE DE BLOCAGE DÉFINITIF SUR L'ÉCRAN D'ACCUEIL */}
        {is_permanently_blocked ? (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5 text-left space-y-2">
            ...
            <p className="text-xs text-red-600 font-medium leading-relaxed">
              Vous avez épuisé le nombre maximal de tentatives autorisées ({attempts_count}) pour cette évaluation. Vous ne pouvez plus relancer cet examen.
            </p>
          </div>
        ) : (
          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-left space-y-3">
            ...
            <ul className="text-xs text-slate-600 space-y-2 font-medium list-disc list-inside">
              ...
              <li>Le chrono démarrera dès que vous aurez cliqué sur le bouton ci-dessous. Assurez-vous d&apos;être disponible.</li>
              <li>Vos réponses sont automatiquement sauvegardées en temps réel.</li>
              <li>Note de validation requise : <strong>{project.passing_score} / 20</strong>.</li>
              {attempts_count > 0 && (
                <li className="text-amber-700 font-semibold">
                  Il s&apos;agit de votre tentative n°{attempts_count + 1}.
                </li>
              )}
            </ul>
          </div>
        ...
        <button
          ...
          disabled={startingSession || is_permanently_blocked}
          className="cursor-pointer w-full py-4 bg-[#000080] hover:bg-blue-900 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-lg hover:shadow-blue-900/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
  ...
  // --- RENDU 1 : SOUMIS / EN CORRECTION / VALIDÉ / ÉCHOUÉ ---
    ...
    return (
      <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-6">
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs text-center space-y-4">
          ...
          {/* Badge récapitulatif du nombre d'essais */}
          {attempts_count > 0 && (
            <div className="inline-block px-3 py-1 bg-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest rounded-full mb-2">
              Tentative n°{attempts_count}
            </div>
          )}
          ...
          {session.status === "submitted" && (
            <>
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                Votre projet d&apos;examen final a été réceptionné et est en attente de correction par le jury.
              </p>
            </>
          )}
          ...
          {session.status === "under_review" && (
            <>
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                Vos livrables sont actuellement en cours d&apos;analyse par nos examinateurs.
              </p>
            </>
          )}
          ...
          {session.status === "approved" && (
            <>
              ...
              <h2 className="text-base font-black text-green-600">Félicitations, Examen Validé !</h2>
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                Votre travail pratique a été validé ! Vous remplissez les critères pour l&apos;obtention de votre certification.
              </p>
            </>
          )}
          ...
          {session.status === "failed" && (
            <>
              ...
              <h2 className="text-base text-red-600 font-bold">Examen non validé</h2>
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                Le score obtenu est en dessous du seuil exigé ({project.passing_score} / 20).
              </p>

              {session.admin_feedback && (
                <div className="mt-4 text-left p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-1">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Retour d&apos;évaluation</span>
                  <p className="text-xs text-slate-700 font-medium leading-relaxed italic">&quot;{session.admin_feedback}&quot;</p>
                </div>
              )}

              {/* CAS DE FIGURE 1: BLOCAGE PERMANENT */}
              {is_permanently_blocked ? (
                <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-2xl space-y-2 text-center">
                  ...
                  <p className="text-xs text-red-600 font-medium">
                    Vous avez atteint le nombre maximal de tentatives d&apos;examen ({attempts_count}). Cet examen est maintenant définitivement bloqué.
                  </p>
                </div>
              ) : isBlockedByTimer ? (
                /* CAS DE FIGURE 2: ATTENTE TIMEOUT (24H) */
                <div className="mt-4 space-y-3">
                  <p className="text-xs text-slate-400 font-medium max-w-sm mx-auto">
                    Prenez le temps de réviser vos cours. Vous pourrez retenter dans :
                  </p>
                  <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-3 flex items-center justify-center gap-2 text-amber-800 font-black text-base tracking-wider">
                    <Clock className="w-4 h-4 text-amber-600" />
                    <span>{formatTime(timeLeftRetry)}</span>
                  </div>
                </div>
              ) : can_retry ? (
                /* CAS DE FIGURE 3: POSSIBILITÉ DE RETENTER */
                <div className="mt-6 space-y-3">
                  <p className="text-xs text-slate-500 font-medium">
                    Prêt à repasser l&apos;examen et tenter de valider votre module ?
                  </p>
                  <button
                    onClick={handleRetryExam}
                    disabled={isRetrying}
                    className="cursor-pointer w-full py-3.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black text-xs uppercase tracking-widest transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
          ...
          {onBackToCourse && (
            <button
              onClick={onBackToCourse}
              className="mt-6 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black uppercase tracking-wider rounded-xl transition-all cursor-pointer"
            >
  ...
  // --- RENDU 2 : FORMULAIRE EXAMEN EN COURS ---
  return (
        ...
        <div className="bg-amber-50 border-2 border-amber-300 rounded-2xl p-4 flex items-start gap-3 text-amber-900 shadow-sm">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs font-medium text-amber-700 leading-relaxed">
              Le temps réglementaire est écoulé. Vous bénéficiez de <strong>30 minutes supplémentaires</strong> pour finaliser et soumettre votre travail.
            </p>
          </div>
        </div>
      )}
