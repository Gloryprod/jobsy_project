...
import { columns, Entreprise } from "./columns";
...
const fetchEntreprises = async (): Promise<Entreprise[]> => {
...
export default function ListEntreprise(){
    const pageLink = `/dashboard/admin/entreprises/list`;
    const queryClient = useQueryClient()
    const { data, error, isLoading } = useQuery<Entreprise[]>({
    ...
    if (error) return (
        <div className="min-h-screen flex items-center justify-center">
            <p className="text-red-400">Erreur de chargement des entreprises.</p>
        </div>
    );
